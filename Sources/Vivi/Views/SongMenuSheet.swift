...
struct SongMenuSheet: View {
    ...
    private var downloadRow: some View {
        if downloads.isDownloaded(song.id) {
            ...
            .background(Color(.secondarySystemBackground))
            .clipShape(RoundedRectangle(cornerRadius: 12, style: .continuous))
        } else if !DownloadManager.isSupported {
            // Kotone: ダウンロードは拡張機能のページコンテキストで
            // fetch して取得する。拡張機能が繋がっていないと使えない。
            //
            // 押しても何も起きないのが一番分かりにくいので、
            // 押せないことと理由を明示する。
            menuRow(icon: "arrow.down.circle",
                    title: "ダウンロード",
                    subtitle: "拡張機能が接続していません",
                    tint: .secondary) {
                showToast("設定 → 診断 → ブラウザ画面を開く で状態を確認できます")
            }
            .disabled(true)
        } else {
            menuRow(icon: "arrow.down.circle",
